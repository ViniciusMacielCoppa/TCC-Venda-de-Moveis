...
<head>
    ...
    <link rel="stylesheet" href="tela_cliente/CLIENTE_avaliacoes.css">
...
<body id="body_avaliacao">
    ...
    <main id="main_principal">
        <div class="container90_main_avaliacoes" id="container_principal_cliente">
            ...
            <!-- ESSA DIV CONTEM O FORMULARIO PARA FAZER UMA AVALIAÇÃO -->
            <?php
                $sql_pedido = "SELECT COUNT(*) AS pedidos_cliente FROM pedidos WHERE user_id = $id AND pedido_status LIKE 5";

                $resultado_pedido = mysqli_query($conexao, $sql_pedido);
                $linhas_pedido = mysqli_num_rows($resultado_pedido);

                for ($i = 0; $i < $linhas_pedido; $i++){
                    $pedidos = mysqli_fetch_array($resultado_pedido);
                    if($pedidos['pedidos_cliente'] >= 1){
                        echo "<style>
                                #body_avaliacao{
                                    padding-bottom: 75vh;
                                }
                            </style>";
                        echo"<div class='fazer_avaliacao_cliente'>
                                <header id='header_avaliacoes_titulo'>
                                    <h2>Faça uma avaliação</h2>
                                </header>
        
                                <main class='fazer_avaliacao_main'>
                                    <form action='cadastrar_pedido.php' method='post'>
                                        <label>
                                            <textarea class='textarea_fazer_avaliacao' name='user_avaliacao' maxlength='500' placeholder='O que achou do nosso serviço?'></textarea>
                                        </label>
                                        <footer id='footer_avaliacoes_conteudo'>
                                            <input type='submit' id='bnt_avaliar' name='avaliar' value='Avaliar'>
